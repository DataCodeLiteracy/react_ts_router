  ...
  return (
    <div className="m-auto w-11/12 md:w-3/4 xl:w-4/5">
      ...
      <div className="flex mt-5">
        <img className="ml-2 mr-3 w-12 rounded-full" src={postDetail.profileImage} alt="" />
        <div>
      ...
      <div className="mt-20 border-t-2">
        {prevPostId > 0 && (
          <Link className="flex items-center text-center mt-10 mr-10" to={`/post/${prevPostId}`}>
            <img className="w-32 h-32 m-auto rounded-lg" src={prevPost.imageUrl} alt="" />
            <h1 className="text-2xl font-bold w-2/3">{prevPost.title}</h1>
          </Link>
        )}
        {nextPostId <= posts.length && (
          <Link className="flex items-center text-center mt-10 mr-10" to={`/post/${nextPostId}`}>
            <img className="w-32 h-32 m-auto rounded-lg" src={nextPost.imageUrl} alt="" />
